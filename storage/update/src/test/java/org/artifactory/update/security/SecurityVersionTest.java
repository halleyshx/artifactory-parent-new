...
public class SecurityVersionTest extends VersionTest {
    ...
    public void convertFromVersion1() throws IOException {
        ...
        assertTrue(admin.isEffectiveAdmin(), "Admin is admin...");
        ...
        assertFalse(momo.isEffectiveAdmin());
    ...
    public void convertFromVersion1Big() throws IOException {
        ...
        for (int i = 0; i != indexOfAdmin && i < users.size(); i++) {
            ...
            assertFalse(user.isEffectiveAdmin(), "Not expecting more admins: " + user.getUsername());
        }
    ...
    public void convertFromVersion8() throws Exception {
        File toConvert = ResourceUtils.getResourceAsFile("/security/v8/security.xml");
        String secXmlData = FileUtils.readFileToString(toConvert, "utf-8");
        String result = SecurityVersion.v8.convert(secXmlData);

        log.debug("convertFromv546 result:\n{}", result);

        SecurityInfo securityInfo = getSecurityInfo(result);

        List<UserInfo> users = securityInfo.getUsers();
        assertEquals(users.size(), 16);

        assertEquals(users.get(0).getPassword(), null);
        assertEquals(users.get(1).getPassword(), "md5$1$$dc16df3b1c37da44dadb835784812123");
        assertEquals(users.get(4).getPassword(), "md5$1$CAFEBABEEBABEFAC$be8a7ba7f2c82b712fb4051192612123");
    }
