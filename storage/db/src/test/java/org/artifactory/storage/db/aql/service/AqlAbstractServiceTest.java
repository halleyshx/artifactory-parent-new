...
public class AqlAbstractServiceTest extends DbBaseTest {
    ...
    protected void assertRepoPathChecksum(AqlEagerResult queryResult, String name, String repoPathChecksum) {
        boolean found = false;
        for (int j = 0; j < queryResult.getSize(); j++) {
    ...
    protected void assertStatisticsRemote(AqlEagerResult queryResult, int downloads, String downloadBy, String origin, String path) {
        boolean found = false;
        for (int j = 0; j < queryResult.getSize(); j++) {
            AqlStatisticItem row = (AqlStatisticItem) queryResult.getResult(j);
