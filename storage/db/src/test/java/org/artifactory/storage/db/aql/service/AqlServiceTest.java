...
public class AqlServiceTest extends AqlAbstractServiceTest {
    ...
    public void findPropertiesUsingJoin() {
        ...
        assertProperty(queryResult, "Geralt", "Of Rivia");
        assertProperty(queryResult, "empty.val", "");
        assertProperty(queryResult, "empty.val", "");
        assertProperty(queryResult, "jungle", "value2");
    ...
    @Test
    public void findBuildWithNotMatchItemProperties() {
        AqlEagerResult queryResult = aqlService.executeQueryEager(
                "builds.find({\"module.artifact.item.repo\" :\"repo1\", \"module.artifact.item.@*\" :{\"$nmatch\":\"*\"}}).include(\"module.artifact.item\")");
        assertSize(queryResult, 0);
    }

    @Test
    public void findArtifactsWithNotMatchProperties() {
        AqlEagerResult queryResult = aqlService.executeQueryEager(
                "items.find({\"repo\" :\"repo1\", \"@*\" :{\"$nmatch\":\"*\"}}).include(\"property\")");
        assertSize(queryResult, 1);
        Assert.assertEquals(((AqlBaseFullRowImpl)queryResult.getResult(0)).getKey(), null);
        Assert.assertEquals(((AqlBaseFullRowImpl)queryResult.getResult(0)).getValue(), null);
    }
    ...
    @Test
    public void itemsFindByRepoPathChecksum() {
        AqlEagerResult queryResult = aqlService.executeQueryEager(
                "items.find({\"created\" : {\"$before\": \"1Days\"}}).include(\"repo_path_checksum\",\"name\").sort({\"$desc\" : [\"name\"]})");
        assertSize(queryResult, 11);
        assertRepoPathChecksum(queryResult, "trustme.jar", "repo-path-name-key-16");
    }

    @Test
    public void includeEmptyItemVirtualRepos() {
        ReflectionTestUtils.setField(aqlService, "repoProvider", new EmptyRepoProvider() {
            ...
            public List<String> getVirtualRepoKeysContainingRepo(String repoKey) {
                switch (repoKey) {
                    case "repo1": return Arrays.asList("v1", "v2");
                    case "repo2": return Arrays.asList("v2", "v3", "v4");
                    case "repo-copy": return Arrays.asList("v2", "v3");
                }
                Assert.fail("Unexpected repo key: " + repoKey);
                return null;
            }
            @Override
        ...
        AqlEagerResult queryResult = aqlService.executeQueryEager(
                "items.find({\"repo\":{\"$match\":\"*\"}}).include(\"repo\",\"path\",\"name\",\"virtual_repos\")");
        assertSize(queryResult, 11);
        queryResult.getResults().stream().forEach(result -> {
            String[] expected = null;
            AqlItem item = (AqlItem)result;
            switch (item.getRepo()) {
                case "repo1": expected = new String[]{"v1"}; break;
                case "repo2": expected = new String[]{"v3", "v4"}; break;
                case "repo-copy": expected = new String[]{"v3"}; break;
            }
            Assert.assertEquals(((AqlItem)result).getVirtualRepos(), expected);
        });
    }

    @Test
    public void includeAllItemsDomainFieldsIncludesAlsoVirtualRepos() {
        AqlEagerResult queryResult = aqlService.executeQueryEager(
                "items.find({\"repo\":{\"$match\":\"*\"}}).include(\"*\")");
        assertSize(queryResult, 11);
        queryResult.getResults().stream().forEach(
                result -> Assert.assertEquals(((AqlItem)result).getVirtualRepos(), new String[0])
        );
    }

    @Test(expectedExceptions = AqlException.class)
    public void includeVirtualReposWithoutAllRepoPathNameFails() {
        aqlService.executeQueryEager("items.find({\"repo\":{\"$match\":\"*\"}}).include(\"repo\",\"virtual_repos\")");
    }
}
