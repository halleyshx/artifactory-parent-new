package org.artifactory.storage.db.upgrades.access;
...
public class SecurityEmigratorFetchersTest extends UpgradeBaseTest {
    ...
    public void testGetAllAclInfos() throws Exception {
        List<AclInfo> allAclInfos = securityEmigratorFetchers.getAllAclInfos();

        assertEquals(allAclInfos.size(), 4);

        AclInfo noam = allAclInfos.stream().filter(acl -> "noam".equals(acl.getUpdatedBy())).findFirst().get();
        ...
        AclInfo permTarget4 = allAclInfos.stream()
