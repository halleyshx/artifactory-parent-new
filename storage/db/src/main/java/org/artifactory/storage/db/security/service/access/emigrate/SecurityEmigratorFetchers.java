...
public class SecurityEmigratorFetchers extends BaseDao {
    ...
    public List<AclInfo> getAllAclInfos() {
        Map<Long, PermissionTargetInfo> targetMap =
    ...
    private List<AclInfo> getAllAclsInfo(Map<Long, PermissionTargetInfo> targetMap) {
        return wrapSqlException("acls", this::getAllAcls).stream()
                .map(acl -> Converters.aclToInfo(
                        wrapSqlException("groups", this::getAllGroupNamePerIds),
                        wrapSqlException("users", this::getAllUsernamePerIds),
                        targetMap,
                        acl))
                .collect(Collectors.toList());
    }
    ...
    private static class Converters {
        ...
        private static PermissionTargetInfo permissionTargetToInfo(PermissionTarget permTarget) {
        ...
        private static AclInfo aclToInfo(Map<Long, String> groups, Map<Long, String> users,
                Map<Long, PermissionTargetInfo> targetMap, Acl acl) {
            return InfoFactoryHolder.get().createAcl(targetMap.get(acl.permTargetId),
                    convertAces(groups, users, acl.aces), acl.lastModifiedBy);
        }
