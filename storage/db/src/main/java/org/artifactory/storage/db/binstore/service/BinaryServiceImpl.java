...
public class BinaryServiceImpl implements InternalBinaryService, ClusterTopologyListener {
    ...
    public BinaryInfo addBinaryRecord(String sha1, String sha2, String md5, long length) {
        try {
            //TODO [by dan][sha2]: switch to sha2 when storage supports it
            ...
            if (result == null) {
                ...
                if (binaryProvider.exists(request).exists()) {
                    ...
                    //TODO [by dan]: if no migration happened yet we're wasting effort here but testing all edge cases isn't worth it
    ...
    private String fillInMissingChecksumInfo(String sha1, String sha2) {
        //TODO [by dan]: when we switch checksum type in the filestore a check for all checksum types must
        //TODO [by dan]: be added here... for now the only possible scenario is export from version < 5.5.0
        //TODO [by dan]: and import into version >= 5.5.0 which means there's no sha2 metadata for the file
        if (isNotBlank(sha2)) {
            return sha2;
        }
        ...
        try (InputStream in = getBinary(element);
            ...
            log.error("Can't calculate missing checksum information for binary with SHA1 '{}' : {}", sha1, e.getMessage());
            log.debug("", e);
        }
        return sha2;
    }
    ...
    @Override
    public BinaryProvidersInfo<Map<String, String>> getBinaryProvidersInfo(boolean useCache){
        // Get binary providers info tree from the binary store manager
        BinaryProvidersInfo binaryProvidersInfo = binaryProviderManager.getBinaryProvidersInfo(useCache);
    ...
    public InputStream getBinary(BinaryInfo bi) {
        BinaryElementRequestImpl request = new BinaryElementRequestImpl(bi.getSha1(), bi.getSha2(), bi.getMd5(), bi.getLength());
        if (!binaryProvider.exists(request).exists()) {
            return null;
        }
    ...
    public InputStream getBinary(BinaryElement element) throws BinaryNotFoundException {
        //TODO [by dan]: BE CAREFUL! users of this method currently supplies the sha1 value -> must refactor ReaderTrackingInputStream in next phase
        return new ReaderTrackingInputStream(binaryProvider.getStream(new BinaryElementRequestImpl(element)), element.getSha1(), this);
    }
    ...
    public BinaryInfo findBinary(ChecksumType checksumType, String checksum) {
        try {
            ...
            throw new StorageException("Storage error loading checksum ( " + checksumType.name() + "): '" + checksum + "'", e);
        }
        return null;
    }
    ...
    public BinaryInfo insertRecordInDb(String sha1, String sha2, String md5, long length) throws StorageException {
        ...
        try {
            //TODO [by dan][sha2]: validate by sha2 as well? can find same-sha1-different-sha2 rows like this and fail uploads
    ...
    private Map<String, String> getExpectedChecksumHeaders(@Nullable ChecksumsInfo info) {
        ...
        if (info != null && ConstantValues.failUploadOnChecksumValidationError.getBoolean()) {
            ...
            addExpectedChecksumHeader(CHECKSUM_SHA256.getHeaderName(), info.getChecksumInfo(ChecksumType.sha256), headers);
     ...
     */
    private void addExpectedChecksumHeader(String headerName, @Nullable ChecksumInfo checksumInfo, Map<String, String> headers) {
