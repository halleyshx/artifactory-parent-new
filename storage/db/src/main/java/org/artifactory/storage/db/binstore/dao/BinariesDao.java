...
public class BinariesDao extends BaseDao {
    ...
    //TODO [by dan]: used by the migration to prevent excessive db calls, remove when we get rid of the migration
    ...
    public Collection<BinaryEntity> findPotentialDeletion() throws SQLException {
        ...
        ResultSet resultSet = null;
        try {
            //TODO [by dan][sha2]: add deletion candidates by sha2 later?
    ...
    protected BinaryEntity binaryFromResultSet(ResultSet rs) throws SQLException {
    ...
    public List<String> getSha1ForMissingSha2(long limit, long offset) throws SQLException {
        ...
        ResultSet resultSet = null;
        try {
            String query = new QueryWriter().select(" sha1 ").from(" binaries ").where(" sha256 IS NULL ")
    ...
    private String resolveSha1ColumnName(JdbcHelper jdbcHelper) {
        ...
        try (Connection connection = jdbcHelper.getDataSource().getConnection()) {
            sha2ColName = DbUtils.normalizedName("sha1", connection.getMetaData());
