...
public class ArtifactoryServersDao extends BaseDao {
    ...
    public int createArtifactoryServer(ArtifactoryServer s) throws SQLException {
        return jdbcHelper.executeUpdate(
                "INSERT INTO artifactory_servers " +
                        "(server_id, start_time, context_url, membership_port, server_state, server_role, last_heartbeat, " +
                        "artifactory_version, artifactory_revision, artifactory_release, artifactory_running_mode, license_hash) " +
                        "VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    public ArtifactoryServer getArtifactoryServer(String serverId) throws SQLException {
        ...
        try {
            resultSet = jdbcHelper.executeSelect("SELECT * FROM artifactory_servers WHERE server_id = ?", serverId);
            if (resultSet.next()) {
    ...
    public int updateArtifactoryServerHeartbeat(String serverId, long lastHeartbeat, String licenseKeyHash) throws SQLException {
        return jdbcHelper.executeUpdate("UPDATE artifactory_servers SET last_heartbeat = ?, license_hash = ? WHERE server_id = ?",
                lastHeartbeat, licenseKeyHash, serverId);
    }
