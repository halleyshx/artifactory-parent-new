...
public class UserGroupsDao extends BaseDao {
    ...
    public boolean isFirstLogin() throws SQLException {
        int userLoginCount = jdbcHelper.executeSelectCount("SELECT COUNT(*) FROM users WHERE last_login_time > 0");
    ...
    public enum GroupFilter {
        ...
        EXTERNAL(" WHERE realm IS NOT NULL AND realm != '" + SecurityConstants.DEFAULT_REALM +
                "' AND realm != 'artifactory'"),
        INTERNAL(" WHERE realm IS NULL OR realm = '" + SecurityConstants.DEFAULT_REALM + "' OR realm = 'artifactory'");
        ...
        try {
            ...
            while (rs.next()) {
                if(filter.test(rs.getLong(4))) {
                    results.add(new PasswordExpiryUser(rs.getString(2), rs.getString(3), rs.getLong(4)));
                }
            }
            return results;
        } finally {
