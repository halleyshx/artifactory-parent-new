...
import static org.artifactory.addon.ha.message.WatchesHaMessage.*;
...
public class WatchesServiceImpl implements WatchesService, InternalWatchesService {
    ...
    public int addWatch(long nodeId, WatcherInfo watchInfo) {
        ...
        HaCommonAddon haCommonAddon = ContextHelper.get().beanForType(AddonsManager.class).addonByType(HaCommonAddon.class);
        String serverId = haCommonAddon.getCurrentMemberServerId();
        notify(new AddWatch(serverId, nodeId, watchInfo));
        return updateCount;
    }
    ...
    private void deleteAllWatchesForRepoPath(RepoPath repoPath) {
        ...
        HaCommonAddon haCommonAddon = ContextHelper.get().beanForType(AddonsManager.class).addonByType(HaCommonAddon.class);
        String serverId = haCommonAddon.getCurrentMemberServerId();
        notify(new DeleteAllWatches(repoPath.getRepoKey(), repoPath.getPath(), repoPath.isFolder(), serverId));
    }
    ...
    private void deleteUserWatchesFromCache(RepoPath repoPath, String username) {
        ...
        HaCommonAddon haCommonAddon = ContextHelper.get().beanForType(AddonsManager.class).addonByType(HaCommonAddon.class);
        String serverId = haCommonAddon.getCurrentMemberServerId();
        notify(new DeleteUserWatches(repoPath.getRepoKey(), repoPath.getPath(), repoPath.isFolder(), username, serverId));
    }
    ...
    private void deleteAllUserWatchesFromCache(String username) {
        ...
        HaCommonAddon haCommonAddon = ContextHelper.get().beanForType(AddonsManager.class).addonByType(HaCommonAddon.class);
        String serverId = haCommonAddon.getCurrentMemberServerId();
        notify(new DeleteAllUserWatches(username, serverId));
    }
    ...
    @Override
    public WatcherInfo watchToWatchInfo(String username, long watchingSinceTime) {
        return new WatcherImpl(username, watchingSinceTime);
    }
    ...
    private void notify(WatchesHaMessage haMessage) {
        ...
        HaWatchMessageWorkItem workitem = new HaWatchMessageWorkItem(haMessage);
