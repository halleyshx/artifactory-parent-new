...
public class BlobBinaryProvider extends BinaryProviderBase {
    ...
    public BinaryElement addStream(BinaryStream binaryStream) throws IOException {
        try {
            ...
            //String sha2 = bd.getSha2();
            if (!internalExists(sha1)) {
                int updated = 0;
                try {
                    //TODO [by dan]: can't insert columns after BLOB type, which means no sha2 in binary_blobs table
                    //TODO [by dan]: without dropping it during conversion :(
