...
public class AqlDao extends BaseDao {
     ...
     */
    public AqlEagerResultImpl executeQueryEager(SqlQuery sqlQuery, AqlRepoProvider aqlRepoProvider) {
     ...
     */
    public AqlLazyResult executeQueryLazy(SqlQuery sqlQuery, AqlPermissionProvider aqlPermissionProvider, AqlRepoProvider aqlRepoProvider) {
        ResultSet resultSet;
        try {
            if (allowReadCommitted()) {
                resultSet = jdbcHelper.executeSelect(sqlQuery.getQueryString(), sqlQuery.getQueryParams());
