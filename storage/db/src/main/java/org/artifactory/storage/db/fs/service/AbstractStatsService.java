...
import static org.artifactory.common.ConstantValues.statsFlushTimeoutSecs;
...
public abstract class AbstractStatsService {
    ...
    public void flushStats() {
        ...
        try {
            boolean lock = getConflictGuard().tryToLock(statsFlushTimeoutSecs.getLong(), TimeUnit.SECONDS);
            if (!lock) {
    ...
    private StatsSaveResult createOrUpdateStats(StatsEvent event) {
        ItemInfo itemInfo;
        try {
            //TODO: [by YS] add {@link java.util.Optional} load item
            itemInfo = getFileService().loadItem(event.getRepoPath());
        ...
        try {
            Stat stats = getStatistics(itemInfo.getId(), event.getOrigin());
            processStats(event, itemInfo.getId(), stats);
     ...
     *
     * @param nodeId   Optional node id ({@link DbService#NO_DB_ID} indicates id is not known yet.
     * @return {@link StatsInfo}
     */
    protected StatsInfo getStatsFromStorage(RepoPath repoPath, long nodeId) {
        if (nodeId <= DbService.NO_DB_ID) {
            // try to fetch the node id from the database
            nodeId = getFileService().getNodeId(repoPath);
        }
        if (nodeId > DbService.NO_DB_ID) {
    ...
    public static StatsInfo statToStatsInfo(Stat stat) {
        ...
        statsInfo.setPath(stat.getPath());
    ...
    protected Stat statInfoToStat(long nodeId, StatsInfo statsInfo) {
        Stat stat = new Stat(
                ...
                statsInfo.getOrigin(), statsInfo.getPath()
        );
        // If stat neither local or remote, very old format, convert to local
        if (!stat.isRemote() && !stat.isLocal()) {
            stat = new Stat(
                    nodeId,
                    statsInfo.getDownloadCount(), statsInfo.getLastDownloaded(), "import",
                    statsInfo.getRemoteDownloadCount(), statsInfo.getRemoteLastDownloaded(), statsInfo.getRemoteLastDownloadedBy(),
                    statsInfo.getOrigin(), statsInfo.getPath()
            );
        }
    ...
    public static class StatsEvent {
        ...
        public String getPath() {
            String pathString;
            if (path.length() != 0 && (pathString = path.toString()).endsWith(PATH_DELIMITER)) {
                return InternalStringUtils.replaceLast(pathString, PATH_DELIMITER, "");
            }
