...
public abstract class UserGroupStoreServiceImpl implements UserGroupStoreService {
        ...
        try {
            ...
            if (originalUser == null) {
                throw new UsernameNotFoundException(
                        "Cannot update user with user name '" + userInfo.getUsername() + "' since it does not exist!");
            }
    ...
    public void encryptDecryptUserProps(String propKey, boolean encrypt) {
        try {
            List<UserProp> userProps = userPropertiesDao.getAllPropertiesByKey(propKey);
            for (UserProp userProp : userProps) {
                String propVal = encrypt ? CryptoHelper.encryptIfNeeded(ArtifactoryHome.get(), userProp.getPropVal())
                        : CryptoHelper.decryptIfNeeded(ArtifactoryHome.get(), userProp.getPropVal());
                userPropertiesDao.updateUserPropertyValue(userProp.getUserId(), userProp.getPropKey(), propVal);
            }
        } catch (SQLException e) {
            log.debug("Could not encrypt or decrypt user props, cause: {}", e);
