...
public class BuildsDao extends BaseDao {
    ...
    public int createBuild(BuildEntity b, BlobWrapper jsonBlob) throws SQLException {
        int res = jdbcHelper.executeUpdate("INSERT INTO builds " +
                        "(build_id, " +
        ...
        res += jdbcHelper.executeUpdate("INSERT INTO build_jsons (build_id, build_info_json) VALUES(?, ?)", b.getBuildId(), jsonBlob);
     ...
     * @throws SQLException
     */
    public BuildEntityRecord getBuildEntityRecord(long buildId) throws SQLException {
        ResultSet rs = null;
    ...
    public List<GeneralBuild> getPrevBuildsList(String buildName, String buildDate) {
        ...
        try {
            String buildsQuery = "SELECT builds.*, build_promotions.status FROM builds " +
                                 "LEFT JOIN build_promotions on build_promotions.build_id = builds.build_id  " +
                                 "WHERE build_name = ? and build_date < ? ORDER BY build_number DESC";
    ...
    public Long getBuildId( BuildParams buildParams) {
        ...
        ResultSet rs = null;
        try{
            rs = jdbcHelper.executeSelect(BuildQueries.BUILD_BY_NUMBER_DATE, diffParams);
            if (rs.next()){
    ...
    public List<BuildIdEntity> getLatestBuildIds(String offset, String orderBy, String direction, String limit) throws SQLException {
        ResultSet rs = null;
        List<BuildIdEntity> buildNames = new ArrayList<>();
        try {
            String allBuildsQuery = "SELECT d.build_id, d.build_name, d.build_number, d.build_date FROM builds d INNER JOIN (SELECT c.build_name, max(c.build_date) build_time from builds c group by c.build_name) k\n" +
            ...
            while (rs.next()) {
                buildId = new BuildIdEntity(rs.getLong(1), rs.getString(2), rs.getString(3), rs.getLong(4));
                buildNames.add(buildId);
            }
        } catch (Exception e) {
            log.error(e.toString());
        } finally {
    ...
    //TODO [by dan]: refactor me with aql!
    ...
    static BuildProperty resultSetToBuildProperty(ResultSet rs) throws SQLException {
        return new BuildProperty(rs.getLong(1), rs.getLong(2),
                rs.getString(3), rs.getString(4));
    }

    static  BuildProps resultSetToBuildProps(ResultSet rs) throws SQLException {
