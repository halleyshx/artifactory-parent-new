...
public class DbConfigFactory implements BeanFactoryAware {
    ...
    public DataSource createUniqueLockDataSource() throws SQLException {
        ArtifactoryDbProperties dbProperties = beanFactory.getBean(ArtifactoryDbProperties.class);
        DataSource result = getDataSourceFromBeanOrJndi(dbProperties, "locks");
        if (result != null) {
            return result;
        } else {
            //TODO [by shayb]: replace all of our data sources and use the common one, but first, talk with Yossi about
            //TODO RTFACT-12728 which it's fix exists in our datasource but missing in the common.
            return getDataSource(dbProperties);
        }
    }

    private DataSource getDataSource(ArtifactoryDbProperties dbProperties) throws SQLException {
        String poolType = dbProperties.getProperty(ArtifactoryDbProperties.Key.poolType);
    ...
    @Bean
    public MetricRegistry getMetricRegistry() throws IOException {
