...
public class TransferEntryTest {
    ...
    /**
     * Creates a new entry "deprecatedstyle" using an entry expression, and validates it
     */

    public void oldDepractedUploadNoIpAddressEntry() {
        String entry = "20050318162747|10|UPLOAD|libs-releases-local:antlr/antlr/2.7.7/antlr-2.7.7.pom|456";
        //String entry = "20050318162747|10|UPLOAD|127.0.0.1|libs-releases-local:antlr/antlr/2.7.7/antlr-2.7.7.pom|456";
        UploadEntry uploadEntry = new UploadEntry(entry);
        assertEquals(uploadEntry.getUserAddress(), "", "Entry content lengths should be equal");

        String builtString = uploadEntry.toString();
        /* remove missing addiress field causing double delimiter */
        builtString = builtString.replace("||", "|");
        assertEquals(entry, builtString, "Upload entry should be equal to the expression that created it");
        assertEquals(uploadEntry.getDuration(), 10, "Duration should be equal");
        assertEquals(uploadEntry.getAction(), TrafficAction.UPLOAD, "Action should be equal");
        assertEquals(uploadEntry.getRepoPath(),
                InfoFactoryHolder.get()
    ...
    @Test(expectedExceptions = IllegalArgumentException.class)
    public void giberishEntry() {
        String entry = "20050318162747|fdkfsadfsjhkdfasjkhdashjlkdfashjdfsahjkdsaFSDA|DFSA|DSF|DSFdsdfsafdsafdsadfs|DFSA|DFSA|DSFD|SF10|UPLOAD|libs-releases-local:antlr/antlr/2.7.7/antlr-2.7.7.pom|456";
        //String entry = "20050318162747|10|UPLOAD|127.0.0.1|libs-releases-local:antlr/antlr/2.7.7/antlr-2.7.7.pom|456";
        UploadEntry uploadEntry = new UploadEntry(entry);
        String builtString = uploadEntry.toString();
    }
