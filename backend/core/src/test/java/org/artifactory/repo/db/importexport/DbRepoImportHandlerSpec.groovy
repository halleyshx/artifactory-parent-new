...
class DbRepoImportHandlerSpec extends Specification {
    ...
    private void setupContext(InternalArtifactoryContext context) {
        def binaryProvidersInfo = new BinaryProvidersInfo<Map<String, String>>("", null)
        binaryService = Mock(BinaryServiceImpl)
        binaryService.getBinaryProvidersInfo(false) >> binaryProvidersInfo
        context.beanForType(BinaryService.class) >> this.binaryService
