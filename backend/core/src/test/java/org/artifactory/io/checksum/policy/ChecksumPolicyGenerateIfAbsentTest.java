...
public class ChecksumPolicyGenerateIfAbsentTest extends ChecksumPolicyBaseTest {
    ...
    public void returnedChecksum() {
        ...
        Assert.assertEquals(policy.getChecksum(sha2), sha2.getActual(), "Should always return the actual value");
    }

    @Override
    void checkShouldVerify() {
        assertTrue(policy.shouldVerifyBadClientChecksum(), "Gen if absent policy requires verification");
    }
}
