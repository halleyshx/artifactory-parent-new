 ...
 * @author Yoav Landman
 */
public class ArtifactoryApplicationContext extends ClassPathXmlApplicationContext implements InternalArtifactoryContext {
    ...
    public void reload(CentralConfigDescriptor oldDescriptor) {
        ...
        try {
            ...
            for (ReloadableBean reloadableBean : reloadableBeans) {
                ...
                log.debug("Reloaded {} in {}. called: {}", getInterfaceName(reloadableBean), (System.currentTimeMillis() - l),
                        callReload);
            }
    ...
    private void exportEtcDirectory(ExportSettings settings) {
        try {
            ...
            if (artifactoryKeyFile.exists()) {
                File targetArtifactoryKeyFile = new File(targetBackupDir, "security" + File.separator + ARTIFACTORY_KEY_DEFAULT_FILE_NAME);
    ...
    private void checkSecurityFolder(File targetBackupDir) throws IOException {
        File artifactoryKeyDest = new File(targetBackupDir, "etc/" + ConstantValues.securityArtifactoryKeyLocation.getDefValue());
    ...
    private void importResourcesFromEtcDirectory(ImportSettings settings) {
        ...
        importSecurityDir(importEtcDir, destSecurityFolder, settings);
    }
    ...
    private void importSecurityDir(File importEtcDir, File destSecurityFolder, ImportSettings settings) {
    ...
    private boolean importingAccessUsers(ImportSettings settings) {
        return settings instanceof ImportSettingsImpl &&
                ((ImportSettingsImpl) settings).getExportVersion().afterOrEqual(ArtifactoryVersion.v560m001);
    }
