...
public class ArchiveIndexerImpl implements InternalArchiveIndexer {
    ...
    public void triggerQueueIndexing() {
        ...
        try {
            boolean indexingSemaphore = getConflictGuard().tryToLock(2, TimeUnit.MINUTES);
            if (!indexingSemaphore) {
                log.trace("Archive indexing already running by another thread");
                return;
            }
            List<RepoPath> indexingQueue = tasksService.getIndexTasks();
            ...
            log.error("Failed to acquire lock for archive index",e);
        } finally {
    ...
    public void recursiveMarkArchivesForIndexing(RepoPath baseRepoPath, boolean indexAllRepos) {
        ...
        for (RepoPath repoPath : toIndex) {
            ItemTree itemTree = new ItemTree(repoPath, item -> item.isFolder() || isIndexSupported(item.getRepoPath()));
    ...
    public boolean isIndexed(RepoPath repoPath) {
        ...
        try {
            ...
            if (!itemInfo.isFolder()) {
                indexed = archiveEntriesService.isIndexed(((FileInfo) itemInfo).getSha1());
            }
    ...
    private void forceArchiveIndexerTaskDeletion(final RepoPath repoPath) {
        dbService.invokeInTransaction("ArchiveIndexer.forceArchiveIndexerTaskDeletion", () -> {
            try {
        ...
        if (conflictGuard == null) {
            synchronized (this) {
                if (conflictGuard == null) {
                    ...
                    conflictGuard = haAddon.getConflictGuard(HaCommonAddon.INDEXING_SEMAPHORE_NAME);
                }
            }
        }
