...
public class BuildSearcher extends SearcherBase {
    ...
    public Set<BuildRun> findBuildsByArtifactChecksum(String sha1, String sha2, String md5) {
        return findBuildsByItemChecksum(BuildSearchCriteria.IN_ARTIFACTS, sha1, sha2, md5);
    }
    ...
    public Set<BuildRun> findBuildsByDependencyChecksum(String sha1, String sha2, String md5) {
        return findBuildsByItemChecksum(BuildSearchCriteria.IN_DEPENDENCIES, sha1, sha2, md5);
    }
     ...
     */
    private Set<BuildRun> findBuildsByItemChecksum(BuildSearchCriteria criteria, String sha1, String sha2, String md5) {
        Set<BuildRun> results = Sets.newHashSet();
        boolean validSha1 = ChecksumType.sha1.isValid(sha1);
        boolean validSha2 = ChecksumType.sha256.isValid(sha2);
        boolean validMd5 = ChecksumType.md5.isValid(md5);

        if (!validSha1 && !validMd5 && !validSha2) {
            return results;
        }
        BuildStoreService buildStoreService = ContextHelper.get().beanForType(BuildStoreService.class);
        if (validSha1) {
        ...
        if (validSha2) {
            results.addAll(buildStoreService.findBuildsForChecksum(criteria, ChecksumType.sha256, sha2));
        }
        if (validMd5) {
