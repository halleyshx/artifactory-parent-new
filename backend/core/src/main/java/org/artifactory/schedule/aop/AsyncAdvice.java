...
public class AsyncAdvice implements MethodInterceptor {
    ...
    public Future<?> invoke(final MethodInvocation invocation) throws Throwable {
        ...
        boolean failIfNotScheduledFromTransaction = asyncMethodAnnotation.annotation.failIfNotScheduledFromTransaction();
        boolean inSession = StorageSessionHolder.getSession() != null;
        validateAnnotations(asyncMethodAnnotation, delayExecutionUntilCommit, failIfNotScheduledFromTransaction, inSession);
        ...
        if (asyncMethodAnnotation.annotation.workQueue()) {
            ...
            traceableInvocation = new TraceableMethodInvocation(invocation, Thread.currentThread().getName(), workExecution);
        } else {
    ...
    private Future<?> submit(final List<TraceableMethodInvocation> workQueueInvocations) {
        workQueueInvocations.forEach(queueInvocation -> submit(queueInvocation.getWorkExecution(), queueInvocation.getMethod()));
        return null;
    }
    ...
    private Future<?> submitWorkQueueTask(WorkQueue<WorkItem> workQueue, WorkItem workItem, Method method) {
        ...
        if (workQueue.availablePermits() != 0) {
            ...
            CachedThreadPoolTaskExecutor executor = InternalContextHelper.get().beanForType(CachedThreadPoolTaskExecutor.class);
    ...
    public static class MethodCallbackSessionResource implements SessionResource {
        ...
        public void addInvocation(TraceableMethodInvocation invocation) {
            if (invocation.getWorkExecution() != null) {
    ...
    public Collection<MethodInvocation> getPendingInvocations() {
    ...
    private void removeInvocationForTest(MethodInvocation invocation) {
        if (ConstantValues.test.getBoolean()) {
