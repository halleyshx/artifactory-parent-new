...
public class TaskServiceImpl implements TaskService, ContextReadinessListener,TaskServiceCallBack {
    ...
    @Override
    public void destroy() {
        ContextHelper.get().beanForType(AsyncWorkQueueProviderService.class).closeAllQueues();
        cancelAllTasks(true);
    ...
    public void onContextReady() {
        Iterator<String> taskTokens = inactiveTasksByToken.keySet().iterator();
        while (taskTokens.hasNext()) {
            String taskKey = taskTokens.next();
            //Check the readiness and task status again since it can change
            if (ContextHelper.get().isReady()) {
                moveToActive(taskKey);
                taskTokens.remove();
            }
        }
    }

    private void moveToActive(String taskKey) {
        TaskBase task = inactiveTasksByToken.get(taskKey);
        if (task != null) {
    ...
    public String startTask(TaskBase task, boolean waitForRunning) {
        ...
        if (context.isOffline()) {
            log.error("Artifactory is in offline state, task "+token+" is not allowed to run during offline state!");
            return token;
        }
        // Verify can be started first
        canBeStarted(task);
        boolean contextReady = context.isReady();
        ConcurrentMap<String, TaskBase> taskMap =
                contextReady ? activeTasksByToken : inactiveTasksByToken;
        if (task.isSingleton()) {
        ...
        // Make sure to use the same ready state
        if (contextReady) {
            task.schedule(waitForRunning);
        } else {
            // Recheck may be readiness was activated in between
            if (context.isReady()) {
                moveToActive(token);
            }
        }
    ...
    @Override
    public void checkCanStartManualTask(Class<? extends TaskCallback> typeToRun, MutableStatusHolder statusHolder, Object... keyValues) {
        ...
        if (currentToken != null) {
            statusHolder.error("Cannot check for manual run with status from inside a running task: " + currentToken, log);
            return;
        }
        ...
        if (impossibleFilter != null) {
            for (TaskBase activeTask : getActiveTasks(impossibleFilter)) {
                if (activeTask.processActive()) {
                    statusHolder.error("Task " + typeToRun.getName() + " cannot stop a mandatory related job " +
                                    activeTask.getType().getName() + " while it's running!", log);
                }
                if (activeTask.getType().equals(
                        typeToRun) && activeTask.isManuallyActivated() && !activeTask.wasCompleted()) {
                    statusHolder.error("Another manual task " + typeToRun.getName() + " is still active!", log);
                }
            }
        }
        ...
        if (stopFilter != null) {
            for (TaskBase activeTask : getActiveTasks(stopFilter)) {
                if (activeTask.processActive()) {
                    statusHolder.warn(
                            "Task " + activeTask.getType().getName() + " will be stop by running "
                                    + typeToRun.getName() + " !", log);
                }
            }
        }
        ...
        if (pauseFilter != null) {
            for (TaskBase activeTask : getActiveTasks(pauseFilter)) {
                if (activeTask.processActive()) {
                    statusHolder.warn(
                            "Task " + activeTask.getType().getName() + " will be paused by running "
                                    + typeToRun.getName() + " !", log);
                }
            }
        }
    ...
    @Override
    public void stopRelatedTasks(Class<? extends TaskCallback> typeToRun, List<String> tokenStopped, Object... keyValues) {
        // Extract task filters for each stop strategy
        EnumMap<StopStrategy, Predicate<Task>> predicatePerStrategy = getPredicatePerStrategy(typeToRun, keyValues);
    ...
    /**
     * Used only by the task in case that the task needs to cancel itself
     */
    @Override
    public void cancelMyselfCallback(TaskBase taskBase){
        activeTasksByToken.remove(taskBase.getToken());
        taskBase.cancelTask();
    }
