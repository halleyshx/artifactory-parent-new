...
public class ArtifactoryStateManagerImpl implements ArtifactoryStateManager, ReloadableBean, ContextReadinessListener {
    ...
    public void init() {
        ...
        if (!runningMode.isOss()) {
            addonsManager.activateLicense(Collections.emptySet(), new LicenseOperationStatus(), false, true);
        }
    ...
    public void onContextCreated() {
        ArtifactoryRunningMode runningMode = addonsManager.getArtifactoryRunningMode();
        if (!runningMode.isOss()) {
            //TODO [by shayb]: Don't do it on AOLs
            boolean success = addonsManager.verifyExistingLicensesOnline(true);
            if (!success) {
                addonsManager.activateLicense(Collections.emptySet(), new LicenseOperationStatus(), false, false);
            }
        }
    }
