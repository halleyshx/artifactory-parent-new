 ...
 * @author Yossi Shaul
 */
@Reloadable(beanClass = InternalLdapAuthenticator.class,
        initAfter = InternalCentralConfigService.class,
        listenOn = { CentralConfigKey.security, CentralConfigKey.proxies })
@Component
public class ArtifactoryLdapAuthenticator implements InternalLdapAuthenticator {
    ...
    static LdapContextSource createSecurityContext(LdapSetting ldapSetting) {
        ...
        if (searchPattern != null) {
            if (PathUtils.hasText(searchPattern.getManagerDn())) {
                ...
                contextSource.setPassword(CryptoHelper.decryptIfNeeded(ArtifactoryHome.get(), searchPattern.getManagerPassword()));
            } else {
    ...
    private Map<String, BindAuthenticator> createBindAuthenticators() {
        ...
        LdapGroupAddon groupAddon = addonsManager.addonByType(LdapGroupAddon.class);
        ...
        for (LdapSetting ldapSetting : ldapSettings) {
            ...
            ArtifactoryBindAuthenticator bindAuthenticator =
                    new ArtifactoryBindAuthenticator(contextSource, ldapSetting, aol);
