...
import org.artifactory.storage.fs.service.*;
...
public class RepositoryServiceImpl implements InternalRepositoryService {
    ...
    private VersioningCache<RepositoriesCache> cache;
    ...
    public void init() {
        if (cache == null) {
            ...
            cache = new VersioningCache<>(timeout, new RepositoriesCacheLoader());
        }
    ...
    public void onContextReady() {
        ...
        if (!gemsAddon.isDefault()) {
            for (LocalRepo localRepo : repositoriesCache.localRepositoriesMap.values()) {
    ...
    @Override
    public FolderSummeryInfo getArtifactCountAndSize(@NotNull RepoPath repoPath) {
    ...
    public Properties getProperties(ItemInfo itemInfo) {
        if (!authService.canRead(itemInfo.getRepoPath())) {
            AccessLogger.downloadDenied(itemInfo.getRepoPath());
            return null;
        }

        MutableVfsItem mutableItem = LockingHelper.getIfWriteLockedByMe(itemInfo.getRepoPath());
        if (mutableItem != null) {
            return mutableItem.getProperties();
        } else {
            return ContextHelper.get().beanForType(PropertiesService.class).getProperties(itemInfo);
        }
    }
     ...
     */
    private BasicStatusHolder undeploySingleItemTransactions(RepoPath repoPath, boolean calcMavenMetadata,
     ...
     */
    private BasicStatusHolder deleteFoldersLeafsBySingleTrx(RepoPath repoPath, boolean calcMavenMetadata,
            BasicStatusHolder statusHolder) {
     ...
     */
    private BasicStatusHolder deleteSingleLeaf(RepoPath repoPath, boolean calcMavenMetadata,
            BasicStatusHolder statusHolder) {
    ...
    public ResourceStreamHandle getResourceStreamHandle(InternalRequestContext requestContext, Repo repo,
        ...
        if (res instanceof ResolvedResource) {
            ...
            if (repo.isReal()) {
                ...
                //TODO [by dan]: checkDownloadIsAllowed is already performed by getInfo() which anyone that gets a
                //TODO RepoResource has to go through we need to see if it can be dropped from here
    ...
    @Override
    @Deprecated //TODO [by dan]: kick this crap out :(
    public String getSha1BySha2Property(String sha2) {
        PropertySearchControls search = new PropertySearchControls();
        search.setSelectedRepoForSearch(new ArrayList<>());
        search.setLimitSearchResults(true);
        search.put("sha256", sha2, true);
        ItemSearchResults<PropertySearchResult> searchResults = searchService.searchPropertyAql(search);
    ...
    private void assertDelete(PathDeletionContext pathDeletionContext) {
        ...
        if (StringUtils.isBlank(path) && !authService.isAdmin()) {
            status.error("Not enough permissions to delete content of repository '" + repo +
                    "' (by user '" + authService.currentUsername() +
                    "'). This requires admin privileges.", HttpStatus.SC_FORBIDDEN, log);
            return;
        }
        RepoPath repoPath = InternalRepoPathFactory.create(repo.getKey(), path);
    ...
    public void updateSha2(ChecksumCalculationWorkItem workItem) {
        ...
        try {
            sha2 = delegate.getOrCalculateSha2(workItem.getSha1());
        } catch (Sha256CalculationFatalException e) {
            //Exception downstream are already logged, just need to mark bad paths.
            delegate.markFatalErrorOnPaths(workItem.getPaths(), e.getMessage());
        }
        if (StringUtils.isNotBlank(sha2)) {
            for (RepoPath path : workItem.getPaths()) {
                ...
                try {
                    ...
                    delegate.incrementTotalDone();
                } catch (ItemNotFoundRuntimeException | FileExpectedException inf) {
                    delegate.log().error("Failed to update sha256 value '" + sha2 + "' on path " + path.toPath(), inf);
                    delegate.markFatalErrorOnPaths(Lists.newArrayList(path), inf.getMessage());
                } catch (Exception e) {
                    delegate.log()
                            .error("Failed to update sha256 value '" + sha2 + "' on path " + path.toPath() + ": ", e);
                    delegate.markFatalErrorOnPaths(Lists.newArrayList(path), e.getMessage());
                }
            }
        }
    }
    ...
    private class RepositoriesCacheBuilder {
        ...
        RepositoriesCache rebuildRepositories() {
            ...
            return new RepositoriesCache(distributionRepositoriesMap, virtualRepositoriesMap, localCacheRepositoriesMap,
                    remoteRepositoriesMap, localRepositoriesMap, trashcan, allRepoKeysCache);
        }
        ...
        private void rebuildRemoteAndCacheRepositories(CentralConfigDescriptor config,
            ...
            for (RemoteRepoDescriptor repoDescriptor : config.getRemoteRepositoriesMap().values()) {
                RemoteRepo remoteRepo = nuGetAddon.createRemoteRepo(transactionalMe, repoDescriptor,
                        config.isOfflineMode(), remoteRepositoriesMap.get(repoDescriptor.getKey()));
                try {
                    remoteRepo.init();
                } catch (Exception e) {
