...
public abstract class BaseRepoPathMover {
    ...
    protected MoveMultiStatusHolder moveOrCopy(VfsItem sourceItem, RepoRepoPath<LocalRepo> targetRrp) {
        targetRrp = adjustTargetPathIfTargetExists(sourceItem, targetRrp);
        moveCopyRecursive(sourceItem, targetRrp);
        postMoveOperations(sourceItem, targetRrp);
        return status;
    }
    ...
    private void postMoveOperations(VfsItem sourceItem, RepoRepoPath<LocalRepo> targetRrp) {
        if (!dryRun) {
     ...
     * see {@link MoverConfig}
     */
    private RepoRepoPath<LocalRepo> adjustTargetPathIfTargetExists(VfsItem sourceItem, RepoRepoPath<LocalRepo> targetRrp) {
    ...
    public MutableVfsFolder shallowCopyDirectory(VfsFolder sourceFolder, RepoRepoPath<LocalRepo> targetRrp) {
        ...
        targetFolder.setProperties((Properties) InfoFactoryHolder.get().copyProperties(sourceFolder.getProperties()));
        replicationAddon.offerLocalReplicationPropertiesChangeEvent(targetRepoPath, false);
        return targetFolder;
    }
    ...
    protected void handleDir(VfsFolder source, RepoRepoPath<LocalRepo> targetRrp) {
        ...
        for (VfsItem child : children) {
            targetRrp = adjustTargetPathToChild(targetRrp, originalRepoPath, child);
    ...
    /**
     * update the cached object with the child's repo path - must know if it's a folder or not for correct
     * include/exclude pattern behavior.
     */
    private RepoRepoPath<LocalRepo> adjustTargetPathToChild(RepoRepoPath<LocalRepo> currentTarget,
            RepoPath originalRepoPath, VfsItem child) {
        return new RepoRepoPath<>(currentTarget.getRepo(), InternalRepoPathFactory.create(originalRepoPath.getRepoKey(),
                originalRepoPath.getPath() + "/" + child.getName(), child.isFolder()));
    }
    ...
    protected boolean canMove(VfsItem source, RepoRepoPath<LocalRepo> targetRrp) {
        ...
        if (contains(targetRrp)) {
            ...
            if (source.isFolder()) {
                VfsItem targetFsItem = targetRepo.getImmutableFsItem(targetRepoPath);
        ...
        if (!storageInterceptors.isCopyOrMoveAllowed(source, targetRepoPath, status)) {
            return false;
        }
    ...
    private void moveFile(VfsFile sourceFile, RepoRepoPath<LocalRepo> targetRrp) {
        assertNotDryRun();
        LocalRepo targetRepo = targetRrp.getRepo();
        RepoPath targetRepoPath = targetRrp.getRepoPath();
        StatusEntry lastError = status.getLastError();
    ...
    void overrideTargetFileIfExist(RepoRepoPath<LocalRepo> targetRrp, LocalRepo targetRepo) {
    ...
    private MutableVfsFile getMutableVfsTargetFile(RepoRepoPath<LocalRepo> targetRrp, LocalRepo targetRepo) {
    ...
    protected void copyVfsFile(VfsFile sourceFile, MutableVfsFile targetFile) {
        ...
        targetFile.tryUsingExistingBinary(sourceFile.getSha1(), sourceFile.getSha2(), sourceFile.getMd5(), sourceFile.length());
        targetFile.fillInfo(sourceFile.getInfo());
        targetFile.setProperties((Properties) InfoFactoryHolder.get().copyProperties(sourceFile.getProperties()));
    }
    ...
    protected void calculateMavenMetadata(RepoRepoPath<LocalRepo> targetRrp, RepoPath sourceFolderRepoPath) {
        ...
        if (calcMetadataOnTarget()) {
            ...
            if (executeMavenMetadataCalculation) {
                mavenMetadataService.calculateMavenMetadataAsync(new MavenMetadataWorkItem(folderForMetadataCalculation, true));
                if (MavenNaming.isPom(fsItem.getRepoPath().getPath())) {
                    ...
                    mavenMetadataService.calculateMavenMetadataAsync(new MavenMetadataWorkItem(grandparentFolder, false));
                }
            } else {
                if (!isTrash(folderForMetadataCalculation)) {
        ...
        if (calcMetadataOnSource() && !isTrash(sourceFolderRepoPath)) {
            ...
            if (!copy && sourceRepo != null && !sourceRepo.isCache() && sourceForMetadataCalculation != null) {
                if (executeMavenMetadataCalculation) {
                    mavenMetadataService.calculateMavenMetadataAsync(new MavenMetadataWorkItem(sourceForMetadataCalculation, true));
                } else {
        ...
        if (localRepo != null) {
            ...
            if (mutableFolder != null) {
                ...
                artifactoryContext.beanForType(AddonsManager.class).addonByType(ReplicationAddon.class)
                        .offerLocalReplicationDeleteEvent(folder.getRepoPath(), false);
            }
        }
    }
