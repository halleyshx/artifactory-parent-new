...
public class ImportHandlerCallablesFactory {
    ...
    ImportHandlerCallablesFactory(Semaphore importsParallelGate, String parentTaskToken) {
    ...
    public Callable<DbRepoImportHandler> create(@Nonnull String repoKey, @Nonnull File repoRoot) {
        ...
        return () -> {
            try {
                DbRepoImportHandler importHandler = new DbRepoImportHandler(localRepo, repoSettings, parentTaskToken);
    ...
    void setBaseImportSettings(ImportSettings baseImportSettings) {
