...
public abstract class DbExportBase extends DbRepoImportExportBase {
    ...
    final ImportExportAccumulator accumulator;

    DbExportBase(ImportExportAccumulator accumulator) {
    ...
    void exportFile(FileInfo sourceFile) {
    ...
    void exportMetadata(File targetFile, ItemInfo sourceItem) {
        try {
    ...
    private void exportItemInfo(ItemInfo itemInfo, File metadataFolder) {
        ...
        writeMetadataToFile(definition, targetMetadataFile, itemInfo, itemInfo.getLastModified());
    }
    ...
    private void exportProperties(ItemInfo itemInfo, File metadataFolder) {
        ...
        File targetMetadataFile = new File(metadataFolder, definition.getMetadataName() + ".xml");
        Properties properties = getPropertiesService().getProperties(itemInfo);
    ...
    private void writeMetadataToFile(MetadataDefinition definition, File targetMetadataFile, Info metadata, long lastModified) {
    ...
    private void writeFile(File metadataFile, String xmlData, long lastModified) {
