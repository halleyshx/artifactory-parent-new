...
public class DbLocalRepo<T extends LocalRepoDescriptor> extends RealRepoBase<T> implements LocalRepo<T> {
    ...
    final DbStoringRepoMixin<LocalRepoDescriptor> mixin;
    ...
    public DbLocalRepo(T descriptor, InternalRepositoryService repositoryService, DbLocalRepo<T> oldLocalRepo) {
        ...
        mixin = new DbStoringRepoMixin<>(descriptor, oldMixin);
    ...
    @Override
    public ResourceStreamHandle getResourceStreamHandle(InternalRequestContext requestContext, RepoResource res) throws IOException {
    ...
    public StatusHolder checkDownloadIsAllowed(RepoPath repoPath) {
        ...
        assertNotBlockedByXray(repoPath, status);
        return status;
    }

    private void assertNotBlockedByXray(RepoPath path, BasicStatusHolder status) {
        if (ContextHelper.get().beanForType(AddonsManager.class).addonByType(XrayAddon.class).isDownloadBlocked(path)) {
            status.error("Artifact download request rejected: " + path.getPath() + " was not downloaded due to "
                            + "the download blocking policy configured in  " + path.getRepoKey()
                            + ".  The artifact may be available for downloading once it has been scanned and "
                            + "indexed by Xray.",
