...
public abstract class RemoteRepoBase<T extends RemoteRepoDescriptor> extends RealRepoBase<T> implements RemoteRepo<T> {
    ...
    private RepoResource getRemoteZipRepoResource(RepoPath repoPath, InternalRequestContext context) {
        try {
            ...
            String alternativeDownloadUrl = context.getRequest().getParameter(PARAM_ALTERNATIVE_REMOTE_DOWNLOAD_URL);
    ...
    private RepoResource getRemoteResource(RequestContext context, RepoPath repoPath, boolean foundExpiredInCache) {
        ...
        if (!isSynchronizeProperties(context) && context.getProperties().hasMandatoryProperty()) {
    ...
    protected void notifyInterceptorsOnAfterRemoteDownload(RepoResource remoteResource) throws RepoRejectException {
        for (RemoteRepoInterceptor interceptor : interceptors) {
     ...
     */
    protected void notifyInterceptorsOnRemoteChecksumResolution(RepoPath repoPath, CloseableHttpResponse response, Set<ChecksumInfo> checksums) {
        for (RemoteRepoInterceptor interceptor : interceptors) {
    ...
    private RepoResource doDownloadAndSave(InternalRequestContext requestContext, RepoResource remoteResource)
        ...
        try {
            ...
            boolean synchronizeProperties = isSynchronizeProperties(requestContext);
    ...
    private boolean isSynchronizeProperties(RequestContext requestContext) {
        if (Boolean.valueOf(requestContext.getRequest().getParameter(PARAM_FORCE_PROPERTIES_SYNC))) {
            return true;
        } else if (getDescriptor().isSynchronizeProperties()) {
            return true;
        } else {
            ContentSynchronisation contentSynchronisation = getDescriptor().getContentSynchronisation();
            if (contentSynchronisation != null) {
                return contentSynchronisation.getProperties().isEnabled();
            }
        }
        return false;
    }
