...
public class MavenMetadataInterceptor extends VirtualRepoInterceptorBase {
    ...
    @Override
    public boolean shouldReturnCachedResource(VirtualRepo virtualRepo, InternalRequestContext requestContext,
            @Nullable RepoResource retrievedResource) {
        //TODO [by dan]: There's no shouldCalculate logic here, i'm leaving it as is to not damage existing behavior
        if (MavenNaming.isIndex(requestContext.getResourcePath())) {
    ...
    private void findAndMergeMavenMetadata(MergeableMavenMetadata mergedMavenMetadata, RealRepo repo,
            InternalRequestContext context, RepoResource res) throws RepoRejectException, BinaryRejectedException {
    ...
    private Metadata getMavenMetadataContent(InternalRequestContext requestContext, Repo repo, RepoResource res)
            throws RepoRejectException, BinaryRejectedException {
