...
public class HttpRepo extends RemoteRepoBase<HttpRepoDescriptor> {
     ...
     */
    private static void addChecksumIfExists(HttpResponse response, ChecksumType type, Set<ChecksumInfo> remoteChecksums) {
     ...
     */
    public CloseableHttpResponse executeMethod(HttpRequestBase method, Map<String, String> extraHeaders) throws IOException {
    ...
    private boolean shouldReplaceHeadWithGet(@Nullable RequestContext requestContext) {
        ...
        if (requestContext != null && requestContext.getRequest() != null) {
            if (StringUtils.isNotBlank(requestContext.getRequest().getParameter(
