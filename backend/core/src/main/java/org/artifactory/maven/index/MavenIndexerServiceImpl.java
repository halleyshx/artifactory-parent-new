...
@Reloadable(beanClass = InternalMavenIndexerService.class,
        ...
        listenOn = CentralConfigKey.indexer)
public class MavenIndexerServiceImpl implements InternalMavenIndexerService {
    ...
    public void mergeVirtualRepoIndexes(@Nonnull Set<? extends RepoDescriptor> includedRepositories,
        ...
        List<VirtualRepo> virtualRepos = getVirtualRepos(indexedRepos.stream().map(RealRepo::getKey).collect(Collectors.toSet()));
     ...
     *
     * @param alreadyIndexedRepositories List of repositories already indexed
     * @return List of virtual repositories to index
     */
    private List<VirtualRepo> getVirtualRepos(@Nonnull Set<String> alreadyIndexedRepositories) {
        ...
        virtualRepositoriesCopy.removeIf(virtualRepo -> alreadyIndexedRepositories.contains(virtualRepo.getKey()));
