...
import org.artifactory.util.stream.MultimapCollectors;
...
public class Sha256MigrationJob extends MigrationJobBase implements Sha256MigrationJobDelegate {
    ...
    protected boolean jobEnabled() {
        return ConstantValues.sha2MigrationJobEnabled.getBoolean();
    }
    ...
    protected String forceExecutionNodeId(JobExecutionContext jobContext) {
        ...
        return ConstantValues.sha2MigrationJobForceRunOnNodeId.getString();
    }

    @Override
    protected ArtifactoryVersion getMinimalVersion() {
        return ArtifactoryVersion.v550m001;
    }
    ...
    protected void logProgress() {
        if (totalNodesDone.get() > nodesMissingSha2Count) {
            ...
            migrationLog.info("SHA256 migration state: {}/{} artifacts were handled.", totalNodesDone.get(), nodesMissingSha2Count);
        }
    }
     ...
     *
     * - Next check is to verify this is the only job running in the cluster. If due to some user error (i.e. setting
     *   {@link ConstantValues#sha2MigrationJobEnabled} and {@link ConstantValues#sha2MigrationJobForceRunOnNodeId}
     *   on 2 nodes) the job is running in more than this node, then this node will fail the job.
     *
    ...
    protected boolean init() {
        if (dbService().verifySha256State()) {
            ...
            migrationLog.debug("SHA256 values already set by previous run of this job, aborting.");
            return false;
        }
        if (refreshCounters()) {
            if (nodesMissingSha2Count < 1 && binariesMissingSha2Count < 1) {
                //first check - no need to run
                log.info("No missing SHA256 values found in database, skipping SHA256 migration job.");
                migrationLog.info("No missing SHA256 values found in database, skipping SHA256 migration job.");
                //TODO [by dan]: if finalize == false maybe try to run anyway?
    ...
    private Multimap<String, RepoPath> getMissingSha2Nodes() {
        ...
        markLastNodeId(missingSha2Nodes);
    ...
    /**
     * Gets the next {@link #queryLimit} results starting from the {@link #lastNodeId} returned by the last query.
    ...
    private AqlEagerResult<AqlItem> queryDbForNextBatch() {
        AqlApiItem query = AqlApiItem.create().filter(
                and(
                        ...
                        AqlApiItem.itemId().greater(lastNodeId)
                ))
    ...
    /**
     * {@link #lastNodeId} is maintained to offset each db query so that we don't reiterate on already-failed nodes.
     */
    private void markLastNodeId(List<FileInfo> missingSha2Nodes) {
        lastNodeId = missingSha2Nodes.stream()
                .map(ItemInfo::getId)
                .max(Comparator.naturalOrder())
                .orElse(-1L);
        migrationLog.debug("Marked last node id = {}", lastNodeId);
    }
        ...
        while(sha1ForMissingSha2.size() > 0) {
            for (String sha1 : sha1ForMissingSha2) {
                ...
                try {
                    ...
                    migrationLog.debug("Binary entries loop interrupted during batch threshold sleep.");
    ...
    protected void markCompletion() {
        migrationLog.debug("SHA256 migration job done, adding NOT NULL constraint to SHA256 column.");
    ...
    protected boolean stateOk() {
        refreshCounters();
        return nodesMissingSha2Count < 1 && binariesMissingSha2Count < 1;
    }
    ...
    public String getOrCalculateSha2(String sha1) throws Sha256CalculationFatalException {
        ...
        if (StringUtils.isNotBlank(sha2)) {
            //gotcha!
            migrationLog.debug("SHA256 value '{}' for sha1 '{}' found from binary entries.", sha2, sha1);
        } else {
            migrationLog.debug("No SHA256 value found in db for sha1 '{}' - running calculation from stream", sha1);
    ...
    private String getSha2FromDb(String sha1) throws Sha256CalculationFatalException {
        ...
        try {
            ...
            //TODO [by dan]: this might not be a fatal error
    ...
    private String calculateSha2AndUpdateDb(String sha1) throws Sha256CalculationFatalException {
        ...
        if (StringUtils.isNotBlank(sha2)) {
            migrationLog.debug("Inserting sha256 value '{}' for sha1 value '{}' into binaries table", sha2, sha1);
            try {
                ...
                //TODO [by dan]: dangerous not to fail here, we can get an edgecase where nodes table is ok but binaries table is missing the sha2
                //TODO [by dan]: can try and recover with separate stage of this job that runs on the binaries table?
                //TODO [by dan]: setting sha2 to null here will trigger another download of the binary though, which is also a shame
                sha2 = null;
            }
        }
        return sha2;
    ...
    private String calculateSha2For(String sha1) throws Sha256CalculationFatalException {
        ...
        migrationLog.debug("Retrieving binary for sha1 '{}' from binary store", sha1);
        ...
        try (InputStream in = binaryService().getBinary(element);
            ...
            migrationLog.debug("Calculating sha256 for sha1 '{}'", sha1);
        ...
        migrationLog.debug("sha256 value '{}' calculated from stream for sha1 '{}'", sha2Value, sha1);
        return sha2Value;
    }
