...
public class ResearchService extends AbstractResearchService {
        ...
        if (!Strings.isNullOrEmpty(httpRepoDescriptor.getUrl())) {
            ...
            if (uri != null) {
                try (CloseableHttpClient client = createHttpClient(httpRepoDescriptor)) {
                    ...
                    log.warn("Error creating research client to url {} : {}", httpRepoDescriptor.getUrl(),
                            ioe.getMessage());
                    log.debug(ioe.getMessage(), ioe);
                }
            } else {
    ...
    private CloseableHttpClient createHttpClient(HttpRepoDescriptor httpRepoDescriptor) {
        HttpClientConfigurator configurator = new HttpClientConfigurator();
        configurator.hostFromUrl(httpRepoDescriptor.getUrl())
                ...
                .authentication(
                        ...
                        CryptoHelper.decryptIfNeeded(ArtifactoryHome.get(), httpRepoDescriptor.getPassword()),
                        httpRepoDescriptor.isAllowAnyHostAuth())
                .enableCookieManagement(httpRepoDescriptor.isEnableCookieManagement());
        return configurator.proxy(httpRepoDescriptor.getProxy()).build();
    }
