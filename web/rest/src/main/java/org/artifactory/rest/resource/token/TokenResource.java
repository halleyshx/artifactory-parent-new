...
import static org.jfrog.access.token.JwtAccessToken.SCOPE_PATTERN;
...
public class TokenResource {
    ...
    private static final Set<String> SIMPLE_REFRESH_TOKEN_REQUEST_PARAMS = ImmutableSet.of("grant_type", "refresh_token", "access_token");
    ...
    public Response createOrRefreshToken(
        ...
        List<String> scopeTokens = scopeToList(scope);
    ...
    private boolean isSimpleRefreshTokenRequestParams() {
        ...
        return SIMPLE_REFRESH_TOKEN_REQUEST_PARAMS.equals(paramNames);
    }
    ...
    @Nullable
    private List<String> scopeToList(String scope) {
        if (isBlank(scope)) {
            return emptyList();
        }
        if (!SCOPE_PATTERN.matcher(scope).matches()) {
            throw new TokenRequestException(InvalidScope, "scope is malformed: " + scope);
        }
        return asList(scope.split(SCOPE_DELIMITER));
    }
