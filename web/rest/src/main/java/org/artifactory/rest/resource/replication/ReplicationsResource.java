...
public class ReplicationsResource extends BaseResource {
    ...
    public Response establishChanel() {
        ...
        try (ServletInputStream inputStream = httpRequest.getInputStream()) {
            ReplicationOwnerModel replicationOwnerModel = JsonUtil.mapDataToModel(inputStream, ReplicationOwnerModel.class);
            String originNode = (replicationOwnerModel.retrieveChannelType().isClusterChannel()) ?
                    httpRequest.getHeader(ARTIFACTORY_HA_ORIGINATED_SERVER_ID) :
                    httpRequest.getHeader("host") + "#" + httpRequest.getHeader(ARTIFACTORY_HA_ORIGINATED_SERVER_ID);
            ReplicationChannel replicationChannel = replicationAddon.establishReplicationChannel(replicationOwnerModel, originNode);
    ...
    public Response handlePullReplicationEvents() throws IOException {
        try {
            ReplicationAddon replicationAddon = addonsManager.addonByType(ReplicationAddon.class);
