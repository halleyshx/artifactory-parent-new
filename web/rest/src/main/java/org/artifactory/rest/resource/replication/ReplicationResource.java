...
public class ReplicationResource {
    ...
    public Response replicateArtifact(FileSpec fileSpec, @HeaderParam(ReplicationRestConstants.DELEGATE_TOKEN_HEADER) String authToken) {
        ...
        BasicStatusHolder statusHolder = distributionAddon.distributeArtifact(fileSpec, authToken);
        if (statusHolder.hasErrors()) {
            int code = statusHolder.getMostImportantErrorStatusCode().getStatusCode();
            return Response.status(code)
                    .entity(ResponseUtils
                            .getResponseWithStatusCodeErrorAndErrorMassages(statusHolder,
                                    getErrMessage(fileSpec), code))
                    .build();
        }

        return Response.ok(ResponseUtils.getStreamingOutput(statusHolder)).build();
    }

    private String getErrMessage(FileSpec fileSpec) {
        return "Error while trying to distribute artifact " + fileSpec.getSourcePath();
    }
