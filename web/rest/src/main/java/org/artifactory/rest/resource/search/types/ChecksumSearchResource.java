...
public class ChecksumSearchResource {
    ...
    public InfoRestSearchResult get(@QueryParam(SearchRestConstants.PARAM_MD5_CHECKSUM) String md5Checksum,
                                    ...
                                    @QueryParam(SearchRestConstants.PARAM_SHA2_CHECKSUM) String sha256Checksum,
        ...
        try {
            ...
            throw mrae;
        } catch (IllegalArgumentException|InvalidChecksumException iae) {
    ...
    private InfoRestSearchResult search(String md5Checksum, String sha1Checksum, String sha256, StringList reposToSearch) throws InvalidChecksumException {
        InfoRestSearchResult resultToReturn = new InfoRestSearchResult();
        Set<RepoPath> matchingArtifacts = restAddon.searchArtifactsByChecksum(md5Checksum, sha1Checksum, sha256, reposToSearch);
        for (RepoPath matchingArtifact : matchingArtifacts) {
            FileInfo fileInfo = repositoryService.getFileInfo(matchingArtifact);
            StorageInfoHelper storageInfoHelper = new StorageInfoHelper(request, repositoryService, repoBrowsingService, fileInfo);
