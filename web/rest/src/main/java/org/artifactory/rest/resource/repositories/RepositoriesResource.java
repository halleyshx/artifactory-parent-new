...
public class RepositoriesResource {
    ...
    private List<RepoDetails> getRepoDetailsList(String repoType) throws Exception {
        ...
        if (noTypeSelected || LOCAL.equals(selectedType)) {
            addNonRemoteRepoDetails(detailsList, repositoryService.getLocalRepoDescriptors(), LOCAL);
        }
        ...
        if (noTypeSelected || VIRTUAL.equals(selectedType)) {
            addNonRemoteRepoDetails(detailsList, repositoryService.getVirtualRepoDescriptors(), VIRTUAL);
        }

        if (noTypeSelected || DISTRIBUTION.equals(selectedType)) {
            addNonRemoteRepoDetails(detailsList, repositoryService.getDistributionRepoDescriptors(), DISTRIBUTION);
        }
        return detailsList;
    }
     ...
     */
    private void addNonRemoteRepoDetails(List<RepoDetails> detailsList, List<? extends RepoDescriptor> reposToAdd,
