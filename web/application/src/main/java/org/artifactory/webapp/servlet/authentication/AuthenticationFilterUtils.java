...
public class AuthenticationFilterUtils {
    ...
    public static boolean acceptSsoFilter(SecurityService securityService, HttpServletRequest request) {
        if (!securityService.isHttpSsoProxied()) {
            return false;
        }

        String ssoUserName = AuthenticationFilterUtils.getRemoteUserName(securityService, request);
