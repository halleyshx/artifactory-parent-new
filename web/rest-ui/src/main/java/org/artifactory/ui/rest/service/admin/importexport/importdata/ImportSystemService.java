...
public class ImportSystemService implements RestService {
    ...
    public void execute(ArtifactoryRestRequest request, RestResponse response) {
        ...
        try {
            ...
            response.error("Failed to perform system import from '" + importFromPath + "'. " +
                    e.getMessage());
            log.error("Failed to import system. " + e.getMessage());
            log.debug("Failed to import system: ", e);
        } finally {
    ...
    private void updateResponsePostImport(RestResponse artifactoryResponse, ImportExportStatusHolder status,
        ...
        if (!warnings.isEmpty()) {
            artifactoryResponse.warn("Warnings have been produced during the import. Please review the logs for further information.");
        }
        List<String> errors = new ArrayList<>();
        if (status.isError()) {
            int errorCount = status.getErrors().size();
            if (errorCount > 1) {
                String msg = errorCount + " errors occurred while importing system from '" + importFromPath + "'. Check logs for more information.";
