...
public class Checksums {
    ...
    public void updateFileInfoCheckSum(FileInfo fileInfo, LocalRepoDescriptor localRepoDescriptor) {
        ...
        if (sha1Info != null) {
            //TODO [by dan]: deprecate this crap
        ...
        if (checksumsOk(md5Info, sha1Info, sha2Info)) {
            ...
            boolean userHasPermissionsToFix = !authService.isAnonymous() && authService.canDeploy(fileInfo.getRepoPath());
    ...
    private ChecksumInfo setChecksum(ChecksumType checksumType, FileInfo fileInfo, boolean isLocalRepo, Consumer<String> setter) {
        String checksum = "";
    ...
    private ChecksumInfo handleSha2(FileInfo fileInfo, boolean isLocalRepo) {
        ...
        if ((checksumInfo == null || StringUtils.isBlank(checksumInfo.getActual()))) {
            ...
            if (ChecksumType.sha256.isValid(sha2FromProperties)) {
                checksumInfo = new ChecksumInfo(ChecksumType.sha256, ChecksumInfo.TRUSTED_FILE_MARKER, sha2FromProperties);
            }
        }
        String checksum = ChecksumType.sha256 + ": Not Calculated";
        if (checksumInfo != null && StringUtils.isNotBlank(checksumInfo.getActual())) {
            checksum = buildChecksumString(checksumInfo, isLocalRepo);
        }
        setSha2(checksum);
        return checksumInfo;
    ...
    private boolean checksumsOk(ChecksumInfo md5Info, ChecksumInfo sha1Info, ChecksumInfo sha2Info) {
        boolean md5Match = md5Info.checksumsMatch();
        boolean sha1Match = sha1Info.checksumsMatch();
        boolean sha2Ok = StringUtils.isNotBlank(sha2Info.getActual()); // match not applicable for sha2
    ...
    private String getSha2FromProperties(RepoPath path) {
        Properties properties = ContextHelper.get().getRepositoryService().getProperties(path);
        String sha2 = null;
        if (properties != null) {
            sha2 = properties.getFirst("sha256");
        }
        return sha2;
    }
