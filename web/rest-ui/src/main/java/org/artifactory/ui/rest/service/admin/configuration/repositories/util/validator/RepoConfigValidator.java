...
public class RepoConfigValidator {
    ...
    public void validateVirtual(VirtualRepositoryConfigModel model, MutableCentralConfigDescriptor configDescriptor) throws RepoConfigException {
    ...
    private void validateVirtualTypeSpecific(TypeSpecificConfigModel model) throws RepoConfigException {
        switch (model.getRepoType()) {
            ...
            case YUM:
                ...
                break;
            case Helm:
                HelmTypeSpecificConfigModel helm = (HelmTypeSpecificConfigModel) model;
                helm.setVirtualRetrievalCachePeriodSecs(Optional.ofNullable(helm.getVirtualRetrievalCachePeriodSecs())
                        .orElse(DEFAULT_VIRTUAL_RETRIEVAL_CACHE_PERIOD));
                break;
            case Chef:
    ...
    public void validateRepoName(String repoKey, boolean isRemote) throws RepoConfigException {
        if (isBlank(repoKey)) {
            throw new RepoConfigException("Repository key cannot be empty", SC_BAD_REQUEST);
        }
        //Local repos limited at 64, remote repos append '-cache' to cache repos which is 6 chars.
        int maxLength = isRemote ? (REPO_KEY_MAX_LENGTH - 6) : REPO_KEY_MAX_LENGTH;
        if (StringUtils.length(repoKey) > maxLength) {
            throw new RepoConfigException("Repository key exceeds maximum length (" + maxLength + ") chars.", SC_BAD_REQUEST);
        }
