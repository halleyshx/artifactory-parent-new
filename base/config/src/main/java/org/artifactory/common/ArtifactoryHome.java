...
public class ArtifactoryHome implements Home {
    ...
    private static final String ARTIFACTORY_KEY_FILE = "artifactory.key";
    ...
    public File getAccessAdminCredsFile() {
        return new File(accessClientDir, normalizedFilesystemPath("keys", ACCESS_CLIENT_CREDS_FILE_NAME));
    }
    ...
    //Based on master.key, not the comm.key
    ...
    public EncryptionWrapper getArtifactoryEncryptionWrapper() {
        ...
        File masterKeyFile = getArtifactoryKey();
        if ((artifactoryEncryptionWrapper == null || artifactoryEncryptionWrapper instanceof DummyEncryptionWrapper) &&
