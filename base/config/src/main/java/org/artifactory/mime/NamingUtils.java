...
public abstract class NamingUtils {
    ...
    private static boolean isPropertiesOrStatisticsOrMavenMetadata(String path) {
        ...
        if (fileName.contains(METADATA_PREFIX)) {
            return isPropertiesMetadata(fileName) ||
                    isStatisticsMetadata(fileName) ||
                    MavenNaming.isMavenMetadataFileName(fileName);
        }
        return false;
    }

    public static boolean isMetadata(String path) {
        //check for the metadata pattern of x/y/z/resourceName:metadataName
        return isPropertiesOrStatisticsOrMavenMetadata(path);
    }
    ...
    /**
     * Returns true if the path points to the deprecated statistics path (i.e., ends with :statistics)
     *
     * @param path The path to check
     * @return True if this is a statistics path
     */
    public static boolean isStatistics(String path) {
        String fileName = PathUtils.getFileName(path);
        if (!isFileNameValid(fileName))
            return false;
    ...
    public static String stripMetadataFromPath(String path) {
        ...
        if (metadataPrefixIdx >= 0) {
            if (isPropertiesOrStatisticsOrMavenMetadata(path)) {
    ...
    private static boolean isPropertiesMetadata(String fileName) {
        return fileName.endsWith(METADATA_PREFIX + "properties");
    }

    private static boolean isStatisticsMetadata(String fileName) {
        return fileName.endsWith(METADATA_PREFIX + "statistics");
    }
