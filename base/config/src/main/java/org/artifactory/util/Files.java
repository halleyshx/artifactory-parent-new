...
public abstract class Files {
    ...
    public static void switchFiles(File oldFile, File newdFile) {
        ...
        if (!newdFile.exists()) {
            ...
            try {
                FileUtils.moveFile(oldFile, newdFile);
            } catch (IOException e) {
        ...
        try {
            FileUtils.moveFile(newdFile, backupOrigFileName);
    ...
    public static void cleanupEmptyDirectories(File directory) {
        cleanupEmptyDirectories(directory, file -> true);
    }
     ...
     *
     * @param directory The directory to cleanup
     * @param filter    Filter to control which empty directories are preserved
     */
    public static void cleanupEmptyDirectories(File directory, Predicate<File> filter) {
        cleanupEmptyDirectories(directory, filter, false);
    }
     ...
     */
    private static void cleanupEmptyDirectories(File directory, Predicate<File> filter, boolean includeBaseDirectory) {
        if (!directory.exists()) {
        ...
        for (File file : files) {
            if (file.isDirectory()) {
                cleanupEmptyDirectories(file, filter, true);  // recursive call always include new base directory
            }
        }

        // all other directories scanned - now check if this direcotry should be deleted
        if (includeBaseDirectory && directory.listFiles().length == 0 && filter.test(directory)) {
    ...
    public static void writeContentToRollingFile(String content, File targetFile) throws IOException {
        if (!targetFile.exists()) {
            ...
            synchronized (targetFile.getPath().intern()) {
                FileUtils.copyFile(targetFile, new File(parentDir, newTargetFileName));
            }
