...
public class ArtifactoryPropertiesTest extends ArtifactoryHomeBoundTest {
    ...
    public void artifactoryVersion() throws URISyntaxException {
        String resourceName = "/config/system/artifactory.properties";
        CompoundVersionDetails versionDetails = ArtifactoryVersionReader.readAndFindVersion(
                ResourceUtils.getResource(resourceName), resourceName);
        ArtifactoryHome.get().getArtifactoryProperties().loadArtifactorySystemProperties(null, versionDetails);
    ...
    public void systemPropsDoesNotOverrideArtifactoryProperties() throws URISyntaxException {
        ...
        CompoundVersionDetails versionDetails = ArtifactoryVersionReader.readAndFindVersion(
                ResourceUtils.getResource(resourceName), resourceName);
        ArtifactoryHome.get().getArtifactoryProperties().loadArtifactorySystemProperties(null, versionDetails);
        // Nope, you can't override what's in there anymore
        assertEquals(ConstantValues.artifactoryVersion.getString(), "10.3");
        assertEquals(ConstantValues.artifactoryRevision.getInt(), 12345);
    }
}
