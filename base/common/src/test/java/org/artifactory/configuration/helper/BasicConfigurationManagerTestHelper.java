...
import static org.jfrog.storage.DbType.DERBY;
...
public class BasicConfigurationManagerTestHelper {
    ...
    public static void createEnvironment(EnvContext envContext) throws IOException {
        // Do cleanup
        ...
        // Fill Db with data
        //TODO [by dan]: access to db should be done separately here not via the adapter (which is not supposed to have db.properties before initialize() is called)
        fillDbWithData(envContext, adapter);
        // Do environment conversion
        basicConfigManagers.initialize();
        // Finally we can start artifactory
        TestingArtifactoryContext artifactoryContext = new TestingArtifactoryContext(basicConfigManagers, configurationManager);
        // Bind Thread with Artifactory context
        ...
        removeEncryptionWrapper(envContext,artifactoryHome);
        // Change the last modified date in the filesystem
        changeLastModifiedToPast(envContext,configurationManager,artifactoryHome);
    }

    private static void removeEncryptionWrapper(EnvContext envContext, ArtifactoryHome artifactoryHome) {
        if (envContext.isRemoveEncryptionWrapper()) {
            artifactoryHome.getMasterKeyFile().delete();
    ...
    public static void setLastModifiedOfConfigToFutureTime(EnvContext envContext,
            String configName) {
        ...
        // Some future timestamp to cheat the config manager to pull this config (+100min)
        long nowPlusSome = System.currentTimeMillis() + 6000000;
        try {
            dbChannel.executeUpdate("UPDATE configs set last_modified = ? WHERE config_name = ?", nowPlusSome,
                    configName);
    ...
    private static CommonDbProperties getCommonDbProperties(ArtifactoryDbProperties envContext) {
        String password = envContext.getPassword();
        String userName = envContext.getUsername();
        String connectionUrl = envContext.getConnectionUrl();
        String dbClass = envContext.getDriverClass();
        return new CommonDbProperties(password, connectionUrl, userName, DERBY, dbClass);
    }
    ...
    public static long touchFileOnFs(String filePath, EnvContext envContext) throws IOException {
    ...
    public static void deleteConfigFile(String filePath, EnvContext envContext) {
        File homeDir = envContext.getHomeDir();
        File file = new File(homeDir, filePath);
        Assert.assertTrue(file.exists());
        file.delete();
