...
import org.jfrog.security.util.MasterKeyUtils;
...
public class ConvertersManagerTestHelper {
    public static final String DEFAULT_MASTER_KEY = "0123456789abcdef0123456789abcdef";

    public static ArtifactoryContext createEnvironment(File home,
            ArtifactoryVersion homeVersion,
            ArtifactoryVersion dbVersion) throws IOException {
        return createEnvironment(home, homeVersion, dbVersion,
                new MockServiceConverter(home + "/.history/db.test", null));
    }

    /**
     * Creates Complete environment files and database simulator
     */
    public static ArtifactoryContext createEnvironment(
            File home,
            ArtifactoryVersion homeVersion,
            ArtifactoryVersion dbVersion,
            ArtifactoryConverter artifactoryConverter) throws IOException {
        if ((homeVersion == null || homeVersion.before(ArtifactoryVersion.v548)) || (dbVersion == null || dbVersion.before(ArtifactoryVersion.v548))) {
        ...
        MasterKeyUtils.initMasterKey(artifactoryHome.getMasterKeyFile(), DEFAULT_MASTER_KEY);
        // Create configuration  manager
        ConfigurationManager configurationManager = ConfigurationManagerImpl.create(
