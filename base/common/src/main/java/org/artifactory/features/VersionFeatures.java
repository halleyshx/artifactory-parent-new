...
import static org.artifactory.version.ArtifactoryVersion.fromVersionString;
...
public class VersionFeatures {
    ...
    public List<VersionFeature> getFeaturesByVersion(VersionInfo version) {
        ...
        if (version == null || (artifactoryVersion = fromVersionString(version.getVersion())) == null) {
            String versionError = version == null ? null : version.getVersion();
            log.error("Artifactory version was not provided or cannot be parsed: {}", versionError);
            return Lists.newLinkedList();
        }
        return features.stream()
                .filter(feature -> isFeatureAvailableInVersion(artifactoryVersion, feature))
    ...
    private boolean isFeatureAvailableInVersion(ArtifactoryVersion artifactoryVersion, VersionFeature feature) {
        ArtifactoryVersion featureVersion = fromVersionString(feature.getAvailableFrom().getVersion());
