...
public class HttpClientConfigurator extends HttpBuilderBase<HttpClientConfigurator> {
    ...
    public CloseableHttpClient build() {
        ...
        CloseableHttpClientDecorator client = (CloseableHttpClientDecorator) super.build();
    ...
    private void configureProxy(ProxyDescriptor proxy) {
        if (proxy != null) {
            ...
            configureProxyNew(proxy);
        }
    }

    public void configureProxyNew(ProxyDescriptor proxy) {
        ProxyConfigBuilder proxyBuilder = proxy(proxy.getHost(), proxy.getPort());
        if (StringUtils.isNotBlank(proxy.getUsername())) {
            if (proxy.getDomain() == null) {
                proxyBuilder.authentication(proxy.getUsername(),
                ...
                try {
                    String ntHost =
                            StringUtils.isBlank(proxy.getNtHost()) ? InetAddress.getLocalHost().getHostName() :
                                    proxy.getNtHost();
                    proxyBuilder.ntlmAuthentication(proxy.getUsername(),
                            CryptoHelper.decryptIfNeeded(ArtifactoryHome.get(), proxy.getPassword()),
                            ntHost, proxy.getDomain());
    ...
    private boolean shouldConfigureBearerDummyCredentials() {
        ...
        if (StringUtils.isNotBlank(proxyHost)) {
            proxyCreds = credsProvider
                    .getCredentials(new AuthScope(proxyHost, AuthScope.ANY_PORT, AuthScope.ANY_REALM));
        }
