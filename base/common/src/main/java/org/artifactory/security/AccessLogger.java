...
public abstract class AccessLogger {
    ...
    public static void loginDenied(Authentication authentication, String username) {
        logAction(null, Action.LOGIN, true, authentication, username, null);
    }
    ...
    public static void logAction(RepoPath repoPath, Action action, boolean denied, Authentication authentication,
            String message) {
        logAction(repoPath, action, denied, authentication, null, message);
    }
    public static void logAction(RepoPath repoPath, Action action, boolean denied, Authentication authentication,
        ...
        if (authentication != null) {
            ...
            if (denied && !isUsername(authentication.getName())) {
                // If the username is not available from the authentication, try to get it from 'username'
                name = isUsername(username) ? username : "NA";
            }
            log.info(
                    (denied ? "[DENIED " : "[ACCEPTED ") + action.name() + "] " + (repoPath != null ? repoPath : "") +
                            (message != null ? message : "") + " for " + name + (address != null ? "/" + address : "") + "."
            );
        }
    }
