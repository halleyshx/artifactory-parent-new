...
public abstract class PropsTokenManager implements TokenManager, EncryptedTokenManager {
    ...
    public TokenKeyValue createToken(String userName) {
        ...
        try {
            ...
            boolean tokenPropCreated = userGroupService.createPropsToken(userName, key, CryptoHelper.encryptIfNeeded(ArtifactoryHome.get(), tokenValue));
            if (tokenPropCreated) {
    ...
    public TokenKeyValue addExternalToken(String userName, String tokenValue) {
        ...
        String key = getPropKey();
        try {
            boolean propsToken = userGroupService.createPropsToken(userName, key, CryptoHelper.encryptIfNeeded(ArtifactoryHome.get(), tokenValue));
            if (propsToken) {
                token = new TokenKeyValue(key, tokenValue);
            }
        } catch (SQLException e) {
    ...
    public TokenKeyValue updateToken(String userName, String value) {
        ...
        TokenKeyValue token = null;
        try {
            boolean propsToken = userGroupService.updatePropsToken(userName, key, CryptoHelper.encryptIfNeeded(ArtifactoryHome.get(), value));
            if (propsToken) {
    ...
    @Override
    public void encryptDecryptAllTokens(boolean encrypt) {
        userGroupService.encryptDecryptUserProps(getPropKey(), encrypt);
    }
}
