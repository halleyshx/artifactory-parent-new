...
public class DistributionCoordinatesResolver extends DistributionCoordinates {
    ...
    @JsonIgnore
    public Set<DistributionRuleToken> tokens = new HashSet<>(); //All related tokens and their values
    ...
    @JsonIgnore
    private Map<DistributionRuleFilterType, CaptureGroupValues> capturingGroups = initCaptureGroups(); //Values of the capturing groups extracted from the rule.
    ...
    private Map<DistributionRuleFilterType, CaptureGroupValues> initCaptureGroups() {
        ...
        Stream.of(DistributionRuleFilterType.values()).forEach((type) -> map.put(type, new CaptureGroupValues()));
     ...
     */
    private DistributionCoordinatesResolver replaceTokensWithValues(DistributionReporter status) {
        try {
            for (DistributionRuleToken token : tokens) {
    ...
    public void addCaptureGroups(DistributionRuleFilterType filterType, Matcher filterMatcher, DistributionReporter status) {
        ...
        if (groupCount > 0) {
            ...
            possibleGroupNames.stream().forEach(groupName -> {
                try {
